  ...
  return (
    <section className="w-full bg-black text-white font-sans border-b-[0.5px] border-white/20">
      <div className="flex flex-col md:flex-row w-full">
        ...
        {/* Left static block */}
        <div className="w-full md:w-56 flex-shrink-0 border-b-[0.5px] md:border-b-0 md:border-r-[0.5px] border-white/20 p-4 h-14 flex items-center justify-center bg-white text-black z-10 relative">
           <span className="text-[10px] uppercase font-bold tracking-widest text-center">Trusted Partners</span>
        </div>
        
        {/* Animated Marquee */}
        <div className="flex-1 overflow-hidden relative flex items-center bg-black h-14">
          ...
          <div className="flex w-max animate-marquee items-center h-full">
            {doubledBrands.map((brand, index) => (
              <div 
                key={index} 
                className="text-2xl font-black uppercase tracking-tighter text-white/30 hover:text-white transition-colors duration-300 px-10"
              >
                {brand}
              </div>
            ))}
