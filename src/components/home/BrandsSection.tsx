        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {brands?.map((brand, i) => (
            <Link key={i} href={`/brands/${brand.slug || '#'}`} className="block group">
              <div className="bg-black/20 backdrop-blur-sm border-y border-white/5 border border-neutral-800 rounded-lg p-6 hover:border-orange-500/50 transition-all duration-300 h-full flex flex-col justify-between">
                <div>
