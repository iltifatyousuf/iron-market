...
export default function InteractiveBackground() {
  ...
  return (
    <div className="fixed inset-0 z-[-1] overflow-hidden bg-[#06090e] pointer-events-none">
      ...
      {/* 4. Interactive Mouse Glow (Highlights the grid) */}
      <div 
        className="absolute inset-0 transition-opacity duration-300"
        style={{
          background: `radial-gradient(600px circle at ${mousePosition.x}px ${mousePosition.y}px, rgba(30, 64, 175, 0.15), transparent 40%)`,
        }}
      />
      
      {/* 5. Interactive Cell Highlight (Square block highlight like the image) */}
      <div 
        className="absolute inset-0 transition-opacity duration-300 opacity-30"
        style={{
          background: `radial-gradient(150px circle at ${mousePosition.x}px ${mousePosition.y}px, rgba(249, 115, 22, 0.1), transparent 100%)`,
          maskImage: 'linear-gradient(#000, #000)', // Ensures mask logic can be applied if needed
          WebkitMaskImage: 'linear-gradient(#000, #000)',
          // This creates the "blocky" snapping effect by snapping the background position to the 144px grid
